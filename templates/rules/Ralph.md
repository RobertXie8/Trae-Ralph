...
- **Git 勾选**: `RALPH_STATE.md` §4（commit/push）与 §5 交付勾选必须与实际 Git/远程状态一致；仅在实际执行成功后再勾选，禁止提前勾选。
...
## 5. 测试通过后的版本入库 (Git Commit & Push)

- **规则**: 当 `05-test-plan.md` 中全部相关用例为 `[x]` 后，必须在 `RALPH_STATE.md` §4 中按顺序完成 **commit** 再 **push**。
- **流程**: `git status`（确认范围）-> `git add` -> `git commit`（信息清晰）-> `git push`（约定分支）；无远程时在 §4 推送项标注 N/A 及原因。
- **禁止**: 严禁在测试未全部通过时勾选 §4/§5；严禁跳过 commit 直接声称已同步远程。
