...
1. **Handoff**: 移交 `ralph-planner`（或按 `RALPH_STATE.md` §4）执行 **Git 同步**：`git status` → `git add` → `git commit` → `git push`，并勾选 §4 checklist。
2. **诚实记录**: 若 `git push` 因权限/无远程失败，不得在 §4 勾选「已推送」；应备注原因或将推送标为 N/A。
